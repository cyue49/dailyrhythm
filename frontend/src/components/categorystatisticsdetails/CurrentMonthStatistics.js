...
const CurrentMonthStatistics = ({ category }) => {
        ...
        <div className='flex flex-col items-start justify-center bg-subCardColor rounded-3xl px-5 py-3 gap-2'>
            ...
            <div className='grid grid-cols-7 grid-flow-row gap-2 w-full p-3 border border-primaryColor rounded-3xl'>
                {weekDaysShort.map((day, index) => (
                    <div key={index} className={`w-full flex flex-row py-2 items-center justify-center font-bold text-sm text-primaryColor border-b border-variantColor`}>{day}</div>
                ))}
                {monthDays.map((day, index) => (
                    <MonthdayCard category={category} day={day} today={today} key={index} />
                ))}
            </div>
