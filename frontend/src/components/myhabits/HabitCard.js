...
const HabitCard = ({ habit, currentDay, selectedCheckOption }) => {
    ...
    return (
        <div className={`w-full flex flex-row flex-nowrap items-center justify-between px-3 py-2 gap-4 rounded-full bg-mainCardColor ${(selectedCheckOption.value === 'check' && dailyCount === 0) ? 'hidden' : ''} ${(selectedCheckOption.value === 'nocheck' && dailyCount !== 0) ? 'hidden' : ''}`}>
            <div className='rounded-full border border-primaryColor bg-mainBgColor hover:bg-primaryColor hover:text-secondaryTextColor cursor-pointer button-animation px-3 py-1' onClick={handleClick}>{dailyCount}</div>
            <div className='flex-1 truncate cursor-pointer' onClick={handleNavigate}>{habit.habit_name}</div>
            <div className='text-xs text-neutralColor pr-2 font-bold'>Total: {totalCount}</div>
        </div>
    )
}
