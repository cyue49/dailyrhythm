        ...
        <div className='flex flex-row flex-nowrap gap-2 border-b-2 border-appGreen w-full py-2'>
            {weekdays.map((day, index) => (
                <div className='center-of-div flex-col flex-1 items-center justify-center' key={index}>
                    ...
                    <div className={`cursor-pointer ${(day.getDate() >= 10) ? 'px-2' : 'px-3'} py-1 rounded-full hover:border-2 hover:border-appGreen button-animation ${(currentDay.getDate() === day.getDate()) ? 'primary-green-button' : ''}`} onClick={() => setCurrentDay(day)}>{day.getDate()}</div>
                </div>
            ))}
        </div>
    )
