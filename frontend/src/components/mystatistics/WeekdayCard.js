...
const WeekdayCard = ({ habit, day }) => {
    ...
    return (
        <div className='center-of-div flex-col flex-1 items-center justify-center gap-2'>
                <div className={`text-sm font-bold text-appGreen w-full flex flex-row justify-center ${habit.weekdays.includes(day.getDay().toString()) ? 'border-b-2 border-appGreen' : 'border-b border-appGray-2'}`}>{getDayLetter(day.getDay())}</div>
